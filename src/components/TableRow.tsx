    ...
    <tbody>
      <tr className="bg-[var(--light)] even:bg-[var(--medium)] hover:bg-[var(--orange)] transition duration-400">
        <td className="lg:table-cell border border-[var(--bg)]">
          <Link href={`/assistidos/${id}`} style={linkStyle}>{name}</Link>
        </td>
        <td className="lg:table-cell border border-[var(--bg)] text-center">
          <Link href={`/assistidos/${id}`} style={linkStyle}>{birth}</Link>
        </td>
        <td className="lg:table-cell border border-[var(--bg)] text-center">
          <Link href={`/assistidos/${id}`} style={linkStyle}>{CPF}</Link>
        </td>
        <td className="lg:table-cell border border-[var(--bg)] text-center">
          <button className="text-[var(--dark)] pb-1 pt-2 px-2 hover:scale-110" onClick={() => removeBeneficiary(id)}>
            <RiDeleteBin5Fill />
          </button>
        </td>
      </tr>
